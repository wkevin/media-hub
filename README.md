...
The backend worker requires environment variables to connect to Cloudflare's AI services.
...
2.  Add the following variables to your `.dev.vars` file. These are used by Wrangler for local development.
    ```ini
    CF_AI_BASE_URL="https://gateway.ai.cloudflare.com/v1/YOUR_ACCOUNT_ID/YOUR_GATEWAY_ID/openai"
    CF_AI_API_KEY="your-cloudflare-api-key"
    ```
    *   Replace `YOUR_ACCOUNT_ID` and `YOUR_GATEWAY_ID` with your Cloudflare details.
    *   The `CF_AI_API_KEY` is a token you create in the Cloudflare dashboard with AI Gateway permissions.
...
2.  **Manual Deployment via CLI:**
    ...
    **Important**: After deploying, you must configure the production environment variables (`CF_AI_BASE_URL`, `CF_AI_API_KEY`) as secrets in your Worker's settings in the Cloudflare dashboard.
